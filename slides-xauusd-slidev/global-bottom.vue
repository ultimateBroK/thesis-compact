...
const opacity = computed<number>(() => +(formatter.value.glowOpacity ?? 0.42))
...
<template>
  <div>
    <div
      ...
      :style="{ filter: `blur(72px) hue-rotate(${hue}deg)` }"
      aria-hidden="true"
    >
      <div
        class="clip bg-gradient-to-r from-[#063145] to-[#042030]"
        :style="{ 'clip-path': `polygon(${poly1})`, opacity: opacity }"
      />
      <div
        class="clip bg-gradient-to-l from-[#06405a] to-[#032a40]"
        :style="{ 'clip-path': `polygon(${poly2})`, opacity: opacity }"
      />
      <div
        class="clip bg-gradient-to-t from-[#caa53d] to-[#f5d77a]"
