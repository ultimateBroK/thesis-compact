...
from hybrid_stacking.config import DATA_DIR, TIMEFRAME, PipelineConfig
...
def build_dataset(config: PipelineConfig) -> pd.DataFrame:
    ...
    featured = add_technical_features(candles)
...
def train_test_time_split(frame: pd.DataFrame, test_size: float = 0.2) -> tuple[pd.DataFrame, pd.DataFrame]:
