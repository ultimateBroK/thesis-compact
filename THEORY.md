...
Mô hình học máy xếp chồng hỗn hợp (Hybrid Stacking) là trung tâm công nghệ của đề tài này. Định hướng "Hybrid" thể hiện việc kết hợp các trường phái học máy khác nhau nhằm tận dụng thế mạnh đặc thù của từng kiến trúc. LSTM học các biểu diễn phụ thuộc thời gian dài hạn và các đặc trưng động từ chuỗi giá lịch sử. Ngược lại, LightGBM và Random Forest khai thác quan hệ phi tuyến trên đặc trưng dạng bảng tĩnh hoặc các chỉ báo kỹ thuật được tính toán thủ công. Phương pháp Stacking cho phép xây dựng một kiến trúc hai tầng. Tầng cơ sở (Level 0) chứa LSTM, LightGBM và Random Forest hoạt động độc lập. Dự báo xác suất của tầng này trở thành thuộc tính đầu vào để huấn luyện Logistic Regression ở tầng siêu học (Level 1 - Meta-learner), nhằm hiệu chuẩn trọng số dự báo và giảm sai lệch tổng thể.
...
Kiến trúc Long Short-Term Memory (LSTM) được sử dụng để trích xuất các đặc trưng động học thời gian từ chuỗi quan sát quá khứ trong một cửa sổ dữ liệu xác định. Cấu trúc của một đơn vị LSTM tiêu chuẩn được định nghĩa bởi hệ thống các cổng toán học sau:
...
Bên cạnh đó, LightGBM và Random Forest đóng vai trò tối ưu hóa việc phân tách phi tuyến tính trên dữ liệu dạng bảng tĩnh và các chỉ báo kỹ thuật. LightGBM tối ưu hóa tốc độ và không gian nhớ nhờ kỹ thuật Exclusive Feature Bundling (EFB) và thuật toán phân tách dựa trên biểu đồ tần suất (Histogram-based). Random Forest huấn luyện nhiều cây quyết định trên các mẫu bootstrap và tập con đặc trưng ngẫu nhiên, từ đó giảm phương sai và tăng độ ổn định trước nhiễu thị trường. Khi kết hợp hai trường phái này, mô hình siêu học (Meta-learner) sử dụng thuật toán Hồi quy Logistic có dạng:
...
| ------------------------- | ------------------------------------------------ | --------------------------------------------------------------------- | -------------------------------------------------------------------- |
| LSTM                      | Bộ trích xuất động lực chuỗi thời gian (Tầng 0). | sequence_length, hidden_units, dropout_rate, learning_rate.           | Tối thiểu hóa sai số entropy chéo đa lớp.                            |
...
| Random Forest             | Bộ giảm phương sai dựa trên tổ hợp cây (Tầng 0). | n_estimators, max_depth, min_samples_leaf.                            | Bootstrap aggregation và bỏ phiếu xác suất.                          |
...
| Giai đoạn 4: Tích hợp hệ thống    | Thiết lập cấu trúc Stacking hai tầng. Tầng 0 huấn luyện LSTM, LightGBM, Random Forest. Tầng 1 sử dụng Logistic Regression.  | PyTorch, lightgbm, scikit-learn. | Kiến trúc mô hình Hybrid Stacking hoàn chỉnh.                           |
